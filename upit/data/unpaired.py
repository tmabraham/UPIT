...
__all__ = ['RandPair', 'TensorImageA', 'TensorImageB', 'ToTensorA', 'ToTensorB', 'change_type_of_tfm',
           'change_type_of_tfm_new', 'get_dls', 'convert_func', 'create_image', 'get_dls_from_hf', 'RandPair',
           'PILImageA', 'PILImageB', 'TensorImageA', 'TensorImageB', 'ToTensorA', 'ToTensorB', 'change_type_of_tfm',
...
class ToTensorA(Transform):
    ...
    order = 5
    def encodes(self, o:PILImage): return TensorImageA(image2tensor(o))


class ToTensorB(Transform):
    "Convert item to TensorImageB"
    order = 5
    def encodes(self, o:PILImage): return TensorImageB(image2tensor(o))
...
def change_type_of_tfm(tfm, old_type, new_type):
    ...
    t_encodes.add(object, type(tfm).encodes.funcs.d.get(old_type).d.get(object)) # add the function
    ...
    t_decodes.add(object, type(tfm).decodes.funcs.d.get(old_type).d.get(object)) # add the function
    return new_tfm

# Cell
def change_type_of_tfm_new(tfm, old_type, new_type):
    assert old_type in new_type.mro() # make sure it's a subclass of the old type
    tfm_mro = tfm.__class_.mro()
    for superclass in tfm_mro:
        if hasattr(superclass, 'encodes') or hasattr(superclass, 'encodes'):
            if bool(geattr(i, 'encodes').funcs.d) or :
                NewTfm = get
...
def get_dls(pathA, pathB, num_A=None, num_B=None, load_size=512, crop_size=256, item_tfms=None, batch_tfms=None, bs=4, num_workers=2, normalize=False):
    ...
    dsets = Datasets(filesA, tfms=[[PILImage.create, ToTensorA, *item_tfms],
                                   [RandPair(filesB),PILImage.create, ToTensorB, *item_tfms]], splits=None)

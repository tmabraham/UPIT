...
__all__ = ['RandPair', 'TensorImage_', 'ToTensor_', 'CustomNormalize', 'get_dls', 'get_dls_from_hf']
...
def get_dls_from_hf(dataset_name, num_A=None, num_B=None, load_size=512, crop_size=256, item_tfms=None, batch_tfms=None, bs=4, num_workers=2, normalize=False):
    ...
    dsets = Datasets(imagesA, tfms=[[np.asarray, PILImage.create, ToTensor, *item_tfms], #np.asarray included as a hack to deal with JpegImageFile
                                   [RandPair(imagesB), np.asarray, PILImage.create, ToTensor_, *item_tfms]], splits=None)
